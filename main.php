			...
			while($row = mysql_fetch_assoc($result)){
				...
				while($row2 = mysql_fetch_assoc($result_comment)){
					...
					if($row2['id']==$_SESSION['userid'])  {// testuser1을 세션 값으로 바꿔야함
						$string .= "<img src= 'img/com_del(default).png' width = '20px' height= '20px' onclick = 'location.href=\"http://localhost:81/deleteComment.php?index=".$row2['com_index']."\"'; onmouseover='this.src=\"img/com_del(over).png\" '; onmouseout='this.src=\"img/com_del(default).png\"';>";
					}
					$string .= "</td>";
					$string .= "</tr>";
